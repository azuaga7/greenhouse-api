...
# Imports de services para acceso directo a datos
from core.db import _conn
from services.invernia_data_query_service import handle_invernia_data_query
from services.history_service import build_series_payload
from services.telemetry_service import build_last_response, build_data_response
from services.alerts_service import build_alerts_state_response, build_alerts_events_response
from services.control_service import build_control_state_response
from services.xlsx_export_service import build_xlsx_range_response
from services.xlsx_service import xlsx_stream, xlsx_stream_multisheet
...
Path(CACHE_API_DIR).mkdir(parents=True, exist_ok=True)
...
async def invernia_data_query(payload: Dict[str, Any]):
    return handle_invernia_data_query(payload, _conn)
...
async def invernia_config_propose(req: InvernIAConfigRequest):
    last_row = build_last_response()
    control_state = build_control_state_response()
...
async def invernia_query(req: InvernIAQueryRequest):
    ...
    if intent == "table":
        if "alert" in (req.prompt or "").lower() or (req.payload and req.payload.dataset == "alerts"):
            alerts = build_alerts_events_response(limit=req.payload.limit if req.payload and req.payload.limit else 100)
            table = _table_from_alerts(alerts)
            return _response(True, "table", "Tabla de alertas generada", table=table)

        rows = build_data_response(limit=req.payload.limit if req.payload and req.payload.limit else DEFAULT_MAX_ROWS)
    ...
    if intent == "chart":
        # Usar data-query layer para obtener datos
        query_payload = {
            "metrics": metrics,
            "windows": windows,
            "max_points": max_points
        }
        window_data = handle_invernia_data_query(query_payload)
    ...
    if intent == "stats":
        ...
        for label, rows in window_data.items():
            for metric in metrics:
                ...
                stat["window"] = label
                stats.append(stat)
        text = await _call_external_ai(req.prompt, req.context.model_dump(), {"stats": stats})
        if not text:
            text = "Resumen estadístico generado a partir de las ventanas solicitadas."
    ...
    last_row = build_last_response()
    control_state = build_control_state_response()
    alerts = build_alerts_events_response(limit=20)
    system_context = {
        "last": last_row,
